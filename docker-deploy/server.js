...
const MASTER_QUESTIONS_FILE = path.join(__dirname, 'data', 'master-questions.json');
const MASTER_CONFIG_FILE = path.join(__dirname, 'data', 'master-config.json');
...
// 读取题库数据
function readMasterQuestions() {
    try {
        if (fs.existsSync(MASTER_QUESTIONS_FILE)) {
            const data = fs.readFileSync(MASTER_QUESTIONS_FILE, 'utf8');
            return JSON.parse(data);
        }
        return null;
    } catch (error) {
...
function readMasterConfig() {
    try {
        if (fs.existsSync(MASTER_CONFIG_FILE)) {
            const data = fs.readFileSync(MASTER_CONFIG_FILE, 'utf8');
            return JSON.parse(data);
        }
        return null;
    } catch (error) {
...
const server = http.createServer((req, res) => {
    ...
    if (pathname === '/api/exam-records') {
        ...
        if (req.method === 'GET') {
            // 获取题库数据
            const data = readMasterQuestions();
            if (data) {
                res.writeHead(200, { 'Content-Type': 'application/json' });
        ...
        if (req.method === 'GET') {
            ...
            if (!filename) {
                res.writeHead(400, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: '缺少file参数' }));
                return;
            }
            
            // 安全检查，只允许访问特定的JSON文件
            const allowedFiles = ['master-questions.json', 'master-config.json', 'exam-records.json'];
            if (!allowedFiles.includes(filename)) {
                res.writeHead(403, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: '不允许访问该文件' }));
                return;
            }
            ...
            if (data) {
                ...
                res.writeHead(404, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, message: `文件${filename}不存在` }));
            }
        } else {
            res.writeHead(405, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ success: false, message: '方法不允许' }));
